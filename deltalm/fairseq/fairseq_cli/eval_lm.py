...
def eval_lm(
    ...
    knnlm: bool = False,
    save_knnlm_dstore: bool = False,
    knn_keytype: Optional[str] = None,
    other_knn_args: Optional[dict] = None,
):
    """
    Args:
    ...
    # TODO: actually pass through the other args
    if knnlm:
        knn_dstore = KNN_Dstore(other_knn_args)

    if save_knnlm_dstore:
        print('keytype being saved:', knn_keytype)
    if other_knn_args.dstore_fp16:
        print('Saving fp16')
        dstore_keys = np.memmap(other_knn_args.dstore_mmap+'_keys.npy', dtype=np.float16, mode='w+', shape=(other_knn_args.dstore_size, other_knn_args.decoder_embed_dim))
        dstore_vals = np.memmap(other_knn_args.dstore_mmap+'_vals.npy', dtype=np.int16, mode='w+', shape=(other_knn_args.dstore_size, 1))
    else:
        print('Saving fp32')
        dstore_keys = np.memmap(other_knn_args.dstore_mmap+'_keys.npy', dtype=np.float32, mode='w+', shape=(other_knn_args.dstore_size, other_knn_args.decoder_embed_dim))
        dstore_vals = np.memmap(other_knn_args.dstore_mmap+'_vals.npy', dtype=np.int, mode='w+', shape=(other_knn_args.dstore_size, 1))
...
def main(cfg: DictConfig, **unused_kwargs):
    ...
    results = eval_lm(
        ...
        knnlm=cfg.knnlm,
        save_knn_dstore=cfg.save_knnlm_dstore,
        knn_keytype=cfg.knn_keytype,
    )
