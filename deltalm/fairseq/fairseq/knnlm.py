# THIS IMPLEMENTATION IS TAKEN FROM https://github.com/urvashik/knnmt/
...
class KNN_Dstore(object):
    def __init__(self, args):
        if isinstance(args, dict):
            # ok not very elegant but whatev
            args = Namespace(**args)
    ...
    def get_knn_scores_per_step(self, queries, vocab_size, pad_idx, use_dtype=torch.float32, save_knns=False, knn_temp=1.0):
        ...
        dists, knns = self.get_knns(queries) # debug: ok here
        ...
        # TODO: seems to be because -1 is in the index? 
        # TODO: this is a quick fix, just ignore -1, investigate later
        #import pdb; pdb.set_trace()
        knn_vals_by_index = knn_vals_by_index.clamp(min=0)
