...
class LanguageModelingConfig(FairseqDataclass):
    ...
    # knnlm related arguments
    knn_keytype: Optional[str] = field(
        default=None, metadata={"help": "for knnlm WT103 results, use last_ffn_input"}
    )

    probe: Optional[int] = field(
        default=8, metadata={"help": "for FAISS, the number of lists to query"}
    )
    
    k: Optional[int] = field(
        default=1024, metadata={"help": "number of nearest neighbors to retrieve"}
    )

    dstore_size: Optional[int] = field(
        default=103227021, metadata={"help": "number of items in the knnlm datastore"}
    )

    dstore_filename: Optional[str] = field(
        default=None, metadata={"help": "File where the knnlm datastore is saved"}
    )

    indexfile: Optional[str] = field(
        default=None, metadata={"help": "File containing the index built using faiss for knn"}
    )

    lmbda: Optional[float] = field(
        default=0.0, metadata={"help": "controls interpolation with knn, 0.0 = no knn"}
    )

    knn_sim_func: Optional[str] = field(
        default=None, metadata={"help": "similarity function to use for knns"}
    )
    
    faiss_metric_type: Optional[str] = field(
        default="l2", metadata={"help": "metric type for faiss"}
    )
    
    no_load_keys: Optional[bool] = field(
        default=False, metadata={"help": "do not load keys"}
    )
    
    dstore_fp16: Optional[bool] = field(
        default=False, metadata={"help": "if true, datastore items are saved in fp16 and int16"}
    )

    move_dstore_to_mem: Optional[bool] = field(
        default=False, metadata={"help": "move the keys and values for knn to memory"}
    )
